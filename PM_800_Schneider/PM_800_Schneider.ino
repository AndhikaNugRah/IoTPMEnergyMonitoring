...
unsigned int registers_ap_800[4]; // Array to store all 4 register values
unsigned int registers_rp_800[4];
...
void loop() {
  ...
  if (newTimer - millis() >= 1000) { // Use millis() directly
    ...
    combinedValueAP = ((int64_t)registers_ap_800[1] );
    float finalValueAP=float(combinedValueAP)*100;//MWH    
    ...
    client.publish("ActivePower_800",  formattedValueAP);
    Serial.print("Combined value (MWH): ");
    Serial.println(formattedValueAP);
    
    int64_t combinedValueRP = 0;
    combinedValueRP = ((int64_t)registers_rp_800[1] );
    float finalValueRP=float(combinedValueRP)*100;//MWH
    // Print the combined value
    ...
    client.publish("ReactivePower_800",  formattedValueRP);    
    Serial.print("Combined value (GVARH): ");
